...
def msra_run(_run, _config, _log):
    ...
    if args.use_tensorboard:
        tb_logs_direc = os.path.join(
            dirname(dirname(abspath(__file__))), "results", "tb_logs", args.env, map_name,
            f"{_config['name']}_{remark_str}",
        )
    ...
    if args.test_encoder:
        result_save_direc = os.path.join(
            dirname(dirname(abspath(__file__))), "results", "recons_logs", args.env, map_name,
            f"{_config['name']}_{remark_str}",
        )
...
def evaluate_sequential(args, runner,mask_model):
    for i in range(args.test_nepisode):
        # assert 0
        runner.run(test_mode=True,mask_model=mask_model)
...
def run_sequential(args, logger):
    ...
    mac = mac_REGISTRY[args.mac](buffer.scheme, groups, args, logger)
    ...
    if args.name == "msra":
        ...
        # add meta model todo
        # args.use_metamask = True
        if args.use_metamask:
            mask_model = FeatureMask(args.ratio, mac.meta_input_shape)
            learner = le_REGISTRY[args.learner](mac, latent_model, buffer.scheme, logger, args, mask_model)
        else:
            mask_model = None
    ...
    if args.checkpoint_path != "":
        ...
        if not os.path.isdir(args.checkpoint_path):
            ...
            return
        print("ceshi",os.listdir(args.checkpoint_path))
        ...
        evaluate_mask_model = FeatureMask(args.ratio, mac.meta_input_shape)
        evaluate_mask_model.load_state_dict(th.load("{}/mask_model.th".format(model_path), map_location=lambda storage, loc: storage))
        ...
        if args.evaluate or args.save_replay:
            ...
            if args.test_encoder:
                ...
                evaluate_sequential(args, runner, mask_model=mask_model)
    ...
    while runner.t_env <= args.t_max:
        ...
        runner.log_episode_num(episode)#
        episode_batch = runner.run(test_mode=False,mask_model=mask_model)
        buffer.insert_episode_batch(episode_batch)
        ...
        if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            for _ in range(n_test_runs):
                runner.run(test_mode=True, teacher_forcing=False,mask_model=mask_model)
        ...
        if args.save_model and (
            ...
            save_path = os.path.join(
                args.local_results_path, "models", args.env, map_name, f"{args.name}_{remark_str}", args.unique_token,
                str(runner.t_env)
            )
        ...
        # episode_duration = episode_start - episode_end
        # print(f"episodes{episode} :{episode_duration:.6f} seconds")
        # episode_total += episode_duration
        # if episode == 50:
        #     episode_total = episode_total/49
        #     print(f"every 50 episodes: {episode_total:.6f} seconds")
